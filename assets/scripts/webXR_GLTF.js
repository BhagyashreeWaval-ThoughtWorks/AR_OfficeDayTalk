...
class WebXR {
  ...
  async onSessionStarted(session) {
    ...
    // to be configured in order to use shadows, see `shared/webXR_util.js`
    ...
    // of Reticle in shared/webXR_util.js for more details.
  ...
  async onClick(e) {
    ...
    if (hits.length) {
      ...
      this.model.scene.position.setFromMatrixPosition(hitMatrix);
      ...
      shadowMesh.position.y = this.model.scene.position.y;

      // Ensure our model has been added to the scene.
      this.scene.add(this.model.scenes[0]);
    }
  }
};

window.app = new WebXR();
